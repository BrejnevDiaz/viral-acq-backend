...
// Replace the main conditionally rendered body with <Routes>
app = app.replace(
    '          {isLoggedIn ? (',
    '          <ScrollToTop />\n          <Routes>\n            <Route path="/" element={\n              isLoggedIn ? ('
);

// Replace the closing of the main condition
app = app.replace(
    '          {/* FOOTER */}',
...
// Inject LegalPage and InfoPage components
const newComponents = `
...
// Modify footer links to use <Link> instead of <a>
// I will just use string replacement on the exact chunks
...
// We have 10 other info links. Let's just do them one by one.
const titles = [
    "Influenceurs TikTok",
    "Coaching Elite",
...
titles.forEach(t => {
    // The previous links look like:
    // <a href="#info" onClick={(e) => { e.preventDefault(); setInfoContent({ title: 'TITLE' }); setShowInfoModal(true); }} style={{ color: '#A1A1AA', fontSize: 14, cursor: 'pointer' }}>TITLE</a>
    // We will just do a regex replace matching the TITLE
...
// Remove the Modals from the bottom
app = app.replace(/\{\/\* Info Modal \*\/\}(.|\n)*\{\/\* Legal Modal \*\/\}(.|\n)*\{\/\* Auth Modal overlay \(Glassmorphism\) \*\/\}/g, '{/* Auth Modal overlay (Glassmorphism) */}');

fs.writeFileSync('src/App.jsx', app, 'utf8');
console.log('Successfully refactored App.jsx to use react-router-dom!');
